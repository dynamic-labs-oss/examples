...
export default function DynamicButton() {
  ...
  const handleVerifyOTP = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(msg.toLowerCase().includes("unauthorized")
        ...
        <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-xl border border-[#DADADA] shadow-lg p-4 z-50">
          {view === "menu" && (
            <div className="space-y-2">
              <p className="text-sm font-medium text-[#030303] mb-3">Sign in to Subscriptions</p>
          ...
          {view === "email" && (
            <form onSubmit={handleSendOTP} className="space-y-3">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" required className="w-full px-3 py-2 text-sm border border-[#DADADA] rounded-lg outline-none focus:border-[#4779FF] text-[#030303]" />
          ...
          {view === "otp" && (
            <form onSubmit={handleVerifyOTP} className="space-y-3">
              ...
              <input type="text" value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="6-digit code" required maxLength={6} className="w-full px-3 py-2 text-sm border border-[#DADADA] rounded-lg outline-none focus:border-[#4779FF] text-[#030303] tracking-widest text-center" />
              {error && <p className="text-xs text-red-500">{error}</p>}
