    ...
    return (
      <div className="relative" ref={panelRef}>
        ...
        >
          ...
          <span className="hidden sm:block font-mono text-xs text-[#606060]">{shortenAddress(solanaAccount.address)}</span>
            ...
            {view !== "export" && (
              <>
                <div className="px-3 py-2 mb-1">
                  ...
                  <p className="text-xs font-mono text-[#030303] truncate mt-0.5">{solanaAccount.address}</p>
                </div>
                <div className="border-t border-[#DADADA] my-1" />
                <button onClick={() => { setView("export"); setError(null); }} className="w-full text-left flex items-center gap-2 px-3 py-2 text-sm text-[#030303] hover:bg-[#F9F9F9] rounded-lg transition-colors">
        ...
        <div className="absolute right-0 top-full mt-2 w-72 bg-white rounded-xl border border-[#DADADA] shadow-lg p-4 z-50">
            ...
            <div className="space-y-2">
              ...
              {getEvmProviders().length > 0 && (
                <>
                  ...
                  <button onClick={() => { setView("wallet"); setError(null); }} className={outlineBtn}><WalletIcon />Connect Solana Wallet</button>
                </>
              )}
          ...
          {view === "email" && (
            <form onSubmit={handleSendOTP} className="space-y-3">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" required className="w-full px-3 py-2 text-sm border border-[#DADADA] rounded-lg outline-none focus:border-[#4779FF] focus:ring-1 focus:ring-[#4779FF]/30 text-[#030303]" />
          ...
          {view === "otp" && (
            <form onSubmit={handleVerifyOTP} className="space-y-3">
              <button type="button" onClick={() => { setView("email"); setError(null); }} className="flex items-center gap-1 text-xs text-[#606060] hover:text-[#030303] mb-1">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m15 18-6-6 6-6"/></svg>Back
              </button>
              <p className="text-sm font-medium text-[#030303]">Enter the code</p>
              <p className="text-xs text-[#606060]">Sent to {email}</p>
              <input type="text" value={otp} onChange={(e) => setOtp(e.target.value)} placeholder="6-digit code" required maxLength={6} className="w-full px-3 py-2 text-sm border border-[#DADADA] rounded-lg outline-none focus:border-[#4779FF] focus:ring-1 focus:ring-[#4779FF]/30 text-[#030303] tracking-widest text-center" />
              {error && <p className="text-xs text-red-500">{error}</p>}
                ...
                getEvmProviders().map((provider) => (
                  <button key={provider.key} onClick={() => handleConnectWallet(provider.key)} disabled={loading} className={outlineBtn}>
                    {provider.metadata.icon && (
                      ...
                      <img src={provider.metadata.icon} alt={provider.metadata.displayName} width={20} height={20} className="rounded-sm shrink-0" />
                    )}
