      ...
      <div className="min-h-screen bg-[rgb(249,249,249)] text-[#030303]">
        <div className="relative overflow-hidden">
          ...
          <div className="relative container mx-auto px-4 pt-20 pb-16 text-center">
            <h1 className="text-5xl sm:text-6xl font-bold mb-5 tracking-tight leading-[1.1] text-[#030303]">
              USDC to cash,{" "}
              <span className="text-[#4779FF]">anywhere</span>
            </h1>
            <p className="text-lg text-[#606060] mb-10 max-w-md mx-auto leading-relaxed">
              Off-ramp your USDC across Base, Ethereum, and Solana. Pick up cash at thousands of locations worldwide.
            </p>

            <div className="mx-auto max-w-xs bg-white border border-[#DADADA] rounded-2xl p-6 text-left space-y-3 shadow-sm">
              {!otpVerification ? (
                <>
                  <input type="email" placeholder="Email address" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSendOtp()} className="w-full bg-[#F9F9F9] border border-[#DADADA] rounded-xl px-4 py-2.5 text-[#030303] placeholder-[#606060] text-sm focus:outline-none focus:border-[#4779FF] transition-colors" suppressHydrationWarning />
                  <button onClick={handleSendOtp} disabled={loading || !email} className="w-full flex items-center justify-center gap-2 bg-[#4779FF] hover:bg-[#3366ee] disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl py-2.5 text-sm font-semibold transition-all duration-200">
                ...
                <>
                  <p className="text-xs text-[#606060] text-center">Code sent to <span className="text-[#030303]">{email}</span></p>
                  <input type="text" inputMode="numeric" placeholder="6-digit code" value={otp} onChange={(e) => setOtp(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleVerifyOtp()} className="w-full bg-[#F9F9F9] border border-[#DADADA] rounded-xl px-4 py-2.5 text-[#030303] placeholder-[#606060] text-sm text-center tracking-widest focus:outline-none focus:border-[#4779FF] transition-colors" maxLength={6} />
                  <button onClick={handleVerifyOtp} disabled={loading || otp.length < 6} className="w-full flex items-center justify-center gap-2 bg-[#4779FF] hover:bg-[#3366ee] disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl py-2.5 text-sm font-semibold transition-all duration-200">
                    {loading ? "Verifying..." : "Verify"}
                    {!loading && <ArrowRight className="w-4 h-4" />}
                  </button>
                  <button onClick={() => { setOtpVerification(null); setOtp(""); }} className="w-full text-[#606060] hover:text-[#030303] text-xs text-center transition-colors py-1">← Back</button>
                </>
              )}
            </div>
          </div>
        ...
        <div className="container mx-auto px-4 py-16">
          <div className="grid md:grid-cols-3 gap-5 max-w-3xl mx-auto">
            {[
              { icon: Wallet, color: "#4779FF", title: "Embedded wallets", desc: "Non-custodial wallets created automatically — no extensions or seed phrases." },
              { icon: Zap, color: "#7c3aed", title: "Multi-chain", desc: "Send USDC on Base, Ethereum, or Solana — switch chains anytime." },
              { icon: Globe, color: "#0284c7", title: "Global pickup", desc: "Thousands of cash pickup locations across 200+ countries." },
            ].map(({ icon: Icon, color, title, desc }) => (
              <div key={title} className="bg-white border border-[#DADADA] rounded-2xl p-6 hover:shadow-sm transition-shadow">
          ...
          <div className="max-w-lg mx-auto mt-20">
            ...
            <div className="relative space-y-0">
              {[
                { step: "01", title: "Sign in", desc: "Authenticate with email — an embedded wallet is created automatically." },
                { step: "02", title: "Choose your chain", desc: "Select Base, Ethereum, or Solana — whichever holds your USDC." },
                { step: "03", title: "Pick up cash", desc: "Enter the amount, complete the flow, and collect cash at a nearby location." },
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="max-w-sm mx-auto space-y-5">
          <div>
            ...
            <p className="text-sm text-[#606060]">Select a network and send USDC for cash pickup.</p>
          </div>
          ...
          <div className="bg-white border border-[#DADADA] rounded-2xl p-6 space-y-5 hover:shadow-sm transition-shadow">
            <div className="flex items-center gap-3">
              ...
              {address && (
                <button onClick={() => { navigator.clipboard.writeText(address); setCopied(true); setTimeout(() => setCopied(false), 2000); }} className="p-1.5 rounded-lg text-[#606060] hover:text-[#030303] hover:bg-[#F9F9F9] transition-colors" title="Copy address">
            ...
            <div className="border-t border-[#DADADA] pt-4">
              ...
              <p className="text-3xl font-bold text-[#030303]">
                {usdcBalance === null ? <span className="text-[#DADADA] text-xl font-normal">Loading...</span> : <>${usdcBalance.toFixed(2)}</>}
              </p>
            </div>

            <button onClick={() => setWidgetOpen(true)} disabled={!address || !usdcBalance} className="w-full flex items-center justify-center gap-2.5 bg-[#4779FF] hover:bg-[#3366ee] disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl py-3 text-sm font-semibold transition-all duration-200">
              <Banknote className="w-4 h-4" />
              Cash Pickup
            </button>
          </div>

          <p className="text-center text-xs text-[#606060]">USDC on {CHAINS[selectedChain].name} → cash at pickup locations worldwide</p>
        </div>
      </div>
